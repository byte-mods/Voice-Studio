...
def _prepare_piper_dataset(
    samples: list[TTSSample], out_dir: Path, target_sr: int, ctx: JobContext, lexicon: dict[str, str] | None = None
) -> Path:
    ...
    with meta_path.open("w", newline="", encoding="utf-8") as fp:
        ...
        for s in samples:
            ...
            # Apply pronunciation overrides using word boundary regex
            processed_text = s.text or ""
            if lexicon:
                import re
            ...
            writer.writerow([s.id, processed_text, s.speaker_id])
...
def tts_finetune_handler(ctx: JobContext) -> dict[str, Any]:
    ...
    _prepare_piper_dataset(samples, dataset_dir, sample_rate, ctx, lexicon=cfg.get("lexicon"))
