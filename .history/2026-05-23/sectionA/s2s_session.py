...
def _resolve_one(
    s, version_id: str | None, fallback: str | None, kind: str
) -> tuple[str, str]:
    if version_id:
...
class _ASR:
    ...
    def transcribe(self, pcm_f32, sr: int) -> str:
...
def _from_pcm16(buf: bytes):
    import numpy as np

    return np.frombuffer(buf, dtype="<i2").astype("float32") / 32768.0


def _chunk_pcm16(arr, frame_ms: int = 80) -> list[bytes]:
...
class S2SSession:
    def __init__(self, cfg: SessionConfig) -> None:
        ...
        self._partial_tasks: set[asyncio.Task] = set()
    ...
    async def _run_reply(self, turn_id: str, cancel: asyncio.Event) -> None:
        try:
            ...
            def gen():
                yield from self._llm.stream_reply(self._history, cancel=cancel)
