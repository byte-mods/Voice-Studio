...
def _make_dataset(
    ...
    class _DS(Dataset):  # type: ignore[misc]
        ...
        def __getitem__(self, idx: int) -> dict[str, Any]:
            ...
            # Apply augmentations (only if config provided)
            if augmentations:
                import numpy as np

                # 1. Speed perturbation
                speed_cfg = augmentations.get("speed", {})
                if speed_cfg.get("enabled", False):
                    min_factor = float(speed_cfg.get("min_factor", 0.9))
                    max_factor = float(speed_cfg.get("max_factor", 1.1))
                    factor = float(np.random.uniform(min_factor, max_factor))
                    if abs(factor - 1.0) > 0.01:
                        old_indices = np.arange(len(data))
                        new_indices = np.linspace(
                            0, len(data) - 1, num=max(1, int(len(data) / factor))
                        )
                        data = np.interp(new_indices, old_indices, data).astype("float32")

                # 2. Reverb / Room simulation (convolve decay impulse)
                reverb_cfg = augmentations.get("reverb", {})
                if reverb_cfg.get("enabled", False):
                    decay_factor = float(reverb_cfg.get("decay", 0.5))
                    # 100ms impulse response at 16kHz
                    rir_len = int(0.1 * 16000)
                    t = np.arange(rir_len)
                    alpha = 25.0 * decay_factor
                    h = np.exp(-alpha * (t / 16000.0))
                    h = h * np.random.normal(0, 0.1, rir_len)
                    h[0] = 1.0
                    data = np.convolve(data, h, mode="same").astype("float32")

                # 3. Noise addition
                noise_cfg = augmentations.get("noise", {})
                if noise_cfg.get("enabled", False):
                    min_snr = float(noise_cfg.get("min_snr_db", 10.0))
                    max_snr = float(noise_cfg.get("max_snr_db", 30.0))
...
def whisper_finetune_handler(ctx: JobContext) -> dict[str, Any]:
    ...
    train_ds = _make_dataset(
        train_samples, processor, max_audio_s, augmentations=cfg.get("augmentations")
    )
