...
def _apply_asr_noise(text: str, error_rate: float) -> str:
    if not text:
        return text
    words = text.split(" ")
    homophones = {
    ...
    for w in words:
        ...
        if random.random() < error_rate:
            ...
            if w_clean in homophones:
                new_w = homophones[w_clean]
                if w[0].isupper():
                    new_w = new_w.capitalize()
                new_words.append(new_w)
            elif len(w_clean) > 2:
                idx = random.randint(0, len(w_clean) - 2)
                lst = list(w_clean)
                ...
                new_words.append(w[:idx] + w[idx+1:])
        else:
            new_words.append(w)
...
def _sanitize_spoken_style(text: str, strip_markdown: bool, strip_emoji: bool) -> str:
    if not text:
        return text
...
def _render_chat(sample: LLMSample, tokenizer: Any, robustness: dict[str, Any] | None = None) -> str:
    messages: list[dict[str, str]] = []
    
    asr_enabled = False
    asr_rate = 0.1
    spoken_enabled = False
    strip_md = True
    strip_em = True
    
    if robustness:
        asr_enabled = bool(robustness.get("asr_noise_enabled", False))
        asr_rate = float(robustness.get("asr_error_rate", 0.1))
        spoken_enabled = bool(robustness.get("spoken_style_enabled", False))
        strip_md = bool(robustness.get("strip_markdown", True))
        strip_em = bool(robustness.get("strip_emoji", True))
    ...
    for turn in sample.turns:
        ...
        if role == "user" and asr_enabled:
            text = _apply_asr_noise(text, asr_rate)
        elif role == "assistant" and spoken_enabled:
            text = _sanitize_spoken_style(text, strip_md, strip_em)
...
def llm_finetune_handler(ctx: JobContext) -> dict[str, Any]:
    ...
    def _to_ds(samples: list[LLMSample], apply_robustness: bool = False) -> HFDataset:
        return HFDataset.from_list([
            {"text": _render_chat(s, tokenizer, robustness=robustness_cfg if apply_robustness else None)}
            for s in samples
        ])

    train_ds = _to_ds(train_samples, apply_robustness=True)
    eval_ds = _to_ds(eval_samples, apply_robustness=False) if eval_samples else None
