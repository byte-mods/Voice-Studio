...
def s2s_native_finetune_handler(ctx: JobContext) -> dict[str, Any]:
    ...
    duplex_mode = bool(cfg.get("duplex_mode", False))
    preserve_voice = bool(cfg.get("preserve_voice", False))
    ...
    if duplex_mode:
        ctx.log("[duplex] Enforcing speaking/listening overlapping attention masks and target response cancellation steps")
    if preserve_voice:
        ctx.log("[voice] Injecting reference speaker vocal mappings (ECAPA-TDNN dimensions) across turns")
